...
from mixed.file_utils import ensure_dir
from mixed.initial_watcher import MixedInitialWatcher
from mixed.followup_watcher import MixedFollowupWatcher
...
def main():
    ...
    print()
    print("=" * 60)
    print("[INFO] mixed 패키지 시작")
    print("[INFO] mixed 입력 폴더: {}".format(INPUT_DIR))
    print("[INFO] mixed 초안 폴더: {}".format(DRAFT_DIR))
    ...
    initial_watcher = MixedInitialWatcher(
        input_dir=INPUT_DIR,
        done_dir=DONE_DIR,
        draft_dir=DRAFT_DIR,
    ...
    followup_watcher = MixedFollowupWatcher(
