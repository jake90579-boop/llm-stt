...
from symptom.llm_client import LLMClient
...
class SymptomEngine:
    ...
    def make_plan(self, user_text):
        ...
        default_obj = {
            "tasks": [
                {
                    "title": "기본 증상 안내",
                    "instruction": "사용자가 말한 증상과 병원에서 도움을 받을 수 있는 일반적 안내를 간단히 정리하라"
                }
            ]
        }
    ...
    def aggregate(self, user_text, worker_results):
        ...
        draft_answer = self.client.generate_text(
            system_prompt="너는 symptom 애그리게이터다.",
    ...
    def finalize(self, original_user_text, draft_answer, followup_answer):
        ...
        final_answer = self.client.generate_text(
            system_prompt="너는 symptom 최종 응답 정리기다.",
            user_prompt=prompt,
            max_output_tokens=500,
        )
